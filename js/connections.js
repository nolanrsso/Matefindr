...
(function(global){
  const CONN_APPS = [
    ...
    {id:'discordserver',name:'Serveur Discord',color:'5865F2',logoId:'discord',prefix:'discord.gg/',placeholder:'code d\'invitation',multi:true},
    ...
    {id:'medal',name:'Medal',color:'F23B49',icon:'medal',prefix:'medal.tv/users/',placeholder:'…'},
  ...
  const ICON_SVG = {
    ...
    medal:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="12" fill="#F23B49"/><path fill="#fff" d="M6.6 16.9V8.3c0-.46.37-.83.83-.83.27 0 .53.13.7.36L12 12.55l3.87-4.72a.9.9 0 0 1 .7-.36c.46 0 .83.37.83.83v8.6a.83.83 0 0 1-1.66 0v-6.1L12.6 14.6a.78.78 0 0 1-1.2 0L8.26 10.8v6.1a.83.83 0 0 1-1.66 0z"/></svg>',
  };
  ...
  function connLogo(app, colorOverride){
    ...
    // logoId : réutilise le slug simpleicons d'un autre id (ex. discordserver → discord,
    // "discordserver" n'existe pas comme slug sur ce CDN).
    return `https://cdn.simpleicons.org/${app.logoId || app.id}/${col}`;
  }
  ...
  /** Renvoie la 1re entrée (types "multi" : plusieurs valeurs sous le même id, ex. discordserver). */
  function connGet(connObj, id){
    if(!connObj || typeof connObj !== 'object' || connIsMetaKey(id)) return null;
    const raw = connObj[id];
    if(Array.isArray(raw)) return raw.length ? connNormalize(raw[0]) : null;
    return connNormalize(raw);
  }

  /** Toutes les entrées d'un id (types "multi") — normalisées, entrées invalides filtrées. */
  function connGetList(connObj, id){
    if(!connObj || typeof connObj !== 'object' || connIsMetaKey(id)) return [];
    const raw = connObj[id];
    if(Array.isArray(raw)) return raw.map(connNormalize).filter(Boolean);
    const single = connNormalize(raw);
    return single ? [single] : [];
  }

  function connIsSet(connObj, id){
    if(connIsMetaKey(id)) return false;
    return connGetList(connObj, id).length > 0;
  }

  /** Ajoute une entrée à un id "multi" (crée le tableau si besoin) — renvoie son index. */
  function connAddEntry(connObj, id, payload){
    if(!connObj || typeof connObj !== 'object' || connIsMetaKey(id)) return -1;
    if(!Array.isArray(connObj[id])) connObj[id] = [];
    connObj[id].push(payload);
    return connObj[id].length - 1;
  }

  function connUpdateEntry(connObj, id, index, payload){
    if(!connObj || typeof connObj !== 'object' || connIsMetaKey(id)) return false;
    if(!Array.isArray(connObj[id]) || !connObj[id][index]) return false;
    connObj[id][index] = payload;
    return true;
  }

  function connRemoveEntryAt(connObj, id, index){
    if(!connObj || typeof connObj !== 'object' || connIsMetaKey(id)) return false;
    if(!Array.isArray(connObj[id])) return false;
    connObj[id].splice(index, 1);
    if(!connObj[id].length) delete connObj[id];
    return true;
  }
  ...
  function buildConnUrl(app, entry){
    ...
    // Lien collé sans protocole (ex. "discord.gg/code") : évite de doubler le
    // domaine avec le prefix ci-dessous ("https://discord.gg/discord.gg/code").
    const domain = (app.prefix || '').replace(/^https?:\/\//i, '').split('/')[0];
    let vNoDomain = v;
    if(domain){
      const noWww = v.replace(/^www\./i, '');
      if(noWww.toLowerCase().startsWith(domain.toLowerCase())) vNoDomain = noWww.slice(domain.length);
    }
    const handle = cleanHandle(vNoDomain);
